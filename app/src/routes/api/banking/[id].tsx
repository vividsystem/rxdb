...
import { bankingId, createBankingSchema } from "~/lib/validation/banking";
...
export async function UPDATE({ request, params }: APIEvent) {
	...
	const banking = await db.update(bankingInfo).set(bodyParser.data).where(eq(bankingInfo, paramsParser.data))		
	if(banking.length < 1) {
		return json({message: "Not found"}, {status: 404})
	}
